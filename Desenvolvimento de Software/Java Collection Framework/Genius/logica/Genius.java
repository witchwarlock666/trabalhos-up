    ...
    public boolean compararCores() {
        for (int i = 0; i < coresJogador.size(); i++) {
            if (coresSorteadas.get(i) != coresJogador.get(i)) {
                ...
                gameOver();
                try {
                    JOptionPane.showMessageDialog(null, "Voce perdeu!\nPontuacao: " + pontos, "Perdeu",
                            JOptionPane.ERROR_MESSAGE);
                } catch (Exception e) {
                    match = true;
                }
                try {
                    if (JOptionPane.showOptionDialog(null, "Deseja continuar?", "Genius", JOptionPane.YES_NO_OPTION,
                            JOptionPane.INFORMATION_MESSAGE, null, null, null) == JOptionPane.NO_OPTION)
                        match = false;
                } catch (Exception e) {
                    match = true;
                }
